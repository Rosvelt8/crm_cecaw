    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Suivez vos performances individuelles et contribuez à la croissance de l'agence.</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="p-6 bg-brand-600 text-white border-none relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-4">
            ...
            <div className="text-4xl font-black italic">{objectifs.length > 0 ? `${atteints}/${objectifs.length}` : '—'}</div>
        ...
        <Card className="p-6 border-2 border-brand-100 bg-brand-50/30 md:col-span-2">
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          <h2 className="text-xl font-bold">Détail des Objectifs</h2>
          <div className="grid gap-4 md:grid-cols-2">
            {objectifs.map((obj) => {
              ...
              return (
                <Card key={obj.id} className="p-5 group">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-sm">{obj.titre ?? obj.nom}</span>
                        {(obj.date_fin ?? obj.dateFin) && (
                          <span className="text-[10px] text-muted-foreground flex items-center gap-1 uppercase tracking-wider">
                            <Calendar className="h-3 w-3" /> Échéance: {new Date(obj.date_fin ?? obj.dateFin).toLocaleDateString('fr-FR')}
                          </span>
                        )}
                        {obj.produit && (
                          <span className="text-[10px] text-brand-600 font-bold flex items-center gap-1 mt-1">
                            <Package className="h-3 w-3" /> Produit: {obj.produit.nom}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-end justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Actuel</span>
                        <span className="text-lg font-black italic">
                          {unite === 'FCFA' ? formatCurrency(actuelle, true) : actuelle} {unite !== 'FCFA' && unite}
                        </span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">Cible</span>
